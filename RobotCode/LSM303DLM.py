...
class LSM303DLM:
	...
	def enableDefault(self):
		...
		self.mag.writeByte(self._LSM303_CRB_REG_M, 0x60) # 011100000
	...
	def readRawAccel(self):
		...
		for i in range(3):
			if(data[i]>2**12/2-1):
				data[i]-=2**12
		self.xa=data[0]*.001*9.8
		self.ya=data[1]*.001*9.8
		self.za=data[2]*.001*9.8
		return data

	def readRawMag(self):
		while(not (self.mag.readByte(0x09)&0x01)):
			pass
	...
	def getHeading(self):
		heading = 180*math.atan2(self.ym, self.xm)/math.pi
		if(heading < 0):
			heading += 360
		return heading

	def getTiltHeading(self):
		pitch = math.asin(-self.xa/9.8)
		roll = math.asin(self.ya/math.cos(pitch))
	
		xh = self.xm*math.cos(pitch)+self.zm*math.sin(pitch)
		yh = self.xm*math.sin(roll)*math.sin(pitch)+self.ym*math.cos(roll)-self.zm*math.sin(roll)*math.cos(pitch)
		zh = -self.xm*math.cos(roll)*math.sin(pitch)+self.ym*math.sin(roll)+self.zm*math.cos(roll)*math.cos(pitch)
